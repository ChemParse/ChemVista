...
from trimesh.voxel import creation as _voxel_creation
...
class Exporter:
    ...
    def _collect_meshes_with_colors(self, printing_mode: bool = False, printing_resolution: int = None) -> List[tuple[trimesh.Trimesh, np.ndarray]]:
        """
        ...
        Args:
            printing_mode: If True, use settings optimized for 3D printing
            printing_resolution: Override resolution for 3D printing (higher = smoother)
        ...
        for obj in self.scene_manager.root.iter_visible():
            ...
            if isinstance(obj, MoleculeObject):
                ...
                settings = vars(obj.render_settings).copy()

                # Override settings for 3D printing mode
                if printing_mode:
                    # Higher resolution for smoother surfaces
                    settings['resolution'] = printing_resolution or 32
                    # Ensure full opacity for solid printing
                    settings['alpha'] = 1.0

                # Create atoms mesh
                atoms_mesh = renderer._create_atoms_mesh(
                    obj.molecule, settings)
                if atoms_mesh is not None:
                    pv_meshes.append(atoms_mesh)

                # Create bonds mesh with center_to_center flag for printing
                bonds_mesh = renderer._create_bonds_mesh(
                    obj.molecule, settings, center_to_center=printing_mode)
                    ...
                    if hasattr(actor, 'GetMapper'):
                        ...
                        if mapper and hasattr(mapper, 'GetInput'):
                            ...
                            if 'RGBA' not in mesh.array_names:
                                ...
                                rgba = np.zeros(
                                    (mesh.n_points, 4), dtype=np.uint8)
                ...
                if 'RGBA' in pv_mesh.array_names:
                    ...
                    if len(rgba.shape) == 1 or rgba.shape[1] == 3:
                        if len(rgba.shape) == 1:
                            ...
                            # Add alpha channel
                            alpha = np.full(
                                (rgba.shape[0], 1), 255, dtype=np.uint8)
    ...
    def export_glb(
        ...
        **kwargs
        ...
        Args:
            ...
            printing_mode: If True, optimize for 3D printing (no gaps, higher resolution, solid)
            printing_resolution: Mesh resolution for 3D printing mode (default: 32)
        ...
        Note:
            When printing_mode=True:
            - Bonds connect center-to-center (no gaps between atoms and bonds)
            - Higher mesh resolution for smoother surfaces
            - All objects are fully opaque (alpha=1.0)
            - Recommended printing_resolution: 32-64 for good quality
        """
        ...
        mode_str = "3D printing" if printing_mode else "visualization"
        logger.info(f"Exporting scene to GLB ({mode_str} mode): {output_path}")
        if printing_mode:
            logger.info(
                f"  3D printing settings: resolution={printing_resolution}, no gaps, solid objects")

        # Collect all meshes with their colors using the specified mode
        meshes_with_colors = self._collect_meshes_with_colors(
            printing_mode=printing_mode,
            printing_resolution=printing_resolution
        )
        ...
        if opaque_meshes:
            if len(opaque_meshes) == 1:
                ...
                combined_opaque_rgba = np.vstack(
                    opaque_colors).astype(np.uint8)

            combined_opaque.visual = ColorVisuals(
                combined_opaque, vertex_colors=combined_opaque_rgba)
        ...
        # Combine transparent meshes (should be none in printing mode)
        if transparent_meshes:
            if len(transparent_meshes) == 1:
                ...
                combined_transparent = trimesh.util.concatenate(
                    transparent_meshes)
                combined_transparent_rgba = np.vstack(
                    transparent_colors).astype(np.uint8)

            combined_transparent.visual = ColorVisuals(
                combined_transparent, vertex_colors=combined_transparent_rgba)
        ...
        try:
            ...
            total_vertices = sum(m.vertices.shape[0]
                                 for m in scene_geometries.values())
            total_faces = sum(m.faces.shape[0]
                              for m in scene_geometries.values())
            ...
            logger.info(
                f"   Meshes: {len(scene_geometries)} ({', '.join(scene_geometries.keys())})")
    ...
    def export_trajectory_animated_glb(
        ...
        **kwargs
        ...
        if cycle_animation and len(frames) > 1:
            ...
            logger.info(
                f"  Cycling enabled: added {len(reverse_frames)} reverse frames")
        ...
        settings_dict = asdict(settings) if hasattr(
            settings, '__dataclass_fields__') else settings
        ...
        all_positions = np.vstack(
            [frame.molecule.positions for frame in frames])
        ...
        if scale == "auto":
            ...
            logger.info(
                f"  Auto-scaling: {max_extent:.2f} Å -> {target_size:.2f} units (scale={scale_factor:.4f})")
        elif scale is not None:
            scale_factor = float(scale)
            logger.info(
                f"  Scale factor: {scale_factor} (max extent: {max_extent:.2f} Å -> {max_extent * scale_factor:.2f} units)")
        else:
            scale_factor = 1.0
        ...
        for atom_idx, (position, symbol) in enumerate(zip(molecule.positions, molecule.get_chemical_symbols())):
            ...
            atom_settings = renderer.atoms_settings.get(
                symbol, renderer.atoms_settings['Unknown'])
        ...
        bonds_faces_list = []
        bonds_colors_list = []
        # Store (atom_a, atom_b, vertex_start, vertex_count) for each bond
        bond_skinning_info = []
        ...
        for bond in bond_list:
            ...
            radius_a = renderer.atoms_settings.get(
                symbol_a, renderer.atoms_settings['Unknown'])['radius']
            radius_b = renderer.atoms_settings.get(
                symbol_b, renderer.atoms_settings['Unknown'])['radius']
            ...
            for cylinder in cylinders:
                ...
                scaled_bond_vertices = (
                    cylinder.points * scale_factor).astype(np.float32)
                bond_vertices_parts.append(scaled_bond_vertices)

                # Adjust face indices for local offset
                cyl_faces = cylinder.faces.reshape(-1,
                                                   4)[:, 1:].astype(np.uint32)
                ...
                if 'RGBA' in cylinder.array_names:
                    ...
                    bond_colors_parts.append(
                        np.full((cylinder.n_points, 4), 200, dtype=np.uint8))
        ...
        if bonds_vertices_list:
            ...
            logger.info(
                f"  Number of bonds with vertices: {len(bond_skinning_info)}")

            # Create skinning data for bonds
            bonds_joints = np.zeros((len(bonds_vertices), 4), dtype=np.uint16)
            bonds_weights = np.zeros(
                (len(bonds_vertices), 4), dtype=np.float32)
            ...
            for bond_info in bond_skinning_info:
                ...
                for v_offset in range(vertex_count):
                    ...
                    rel_pos = v_pos - pos_a
                    t = np.dot(rel_pos, bond_dir) / \
                        bond_length if bond_length > 1e-6 else 0.5
        ...
        if bonds_vertices is not None:
            ...
            atoms_weights = np.zeros(
                (len(atoms_vertices), 4), dtype=np.float32)
        ...
        times = np.array(
            [i / fps for i in range(num_frames)], dtype=np.float32)
        ...
        inv_bind_offset, inv_bind_len = add_to_buffer(
            inv_bind_col_major.tobytes())
        ...
        for atom_idx in range(num_atoms):
            translations = np.array(
                [positions[atom_idx] for positions in atom_positions_per_frame], dtype=np.float32)
        ...
        try:
            ...
            logger.info(
                f"✅ Successfully exported animated trajectory to {output_path}")
            logger.info(f"   File size: {total_length / 1024:.1f} KB")
            logger.info(
                f"   Animation: {num_frames} frames at {fps} fps = {num_frames/fps:.1f}s")
        except Exception as e:
    ...
    def export_animated_glb(
        ...
        **kwargs
        ...
        if scalar_fields:
            ...
            print(
                f"⚠️  Warning: {len(scalar_fields)} scalar field(s) skipped (not supported in animated export)")

        # Check what we have to export
        if not trajectories and not molecules:
            raise ValueError(
                "No molecules or trajectories found in scene to export")
        ...
        if cycle_animation and num_frames > 1:
            ...
            # Add reverse frames excluding endpoints
            num_frames = num_frames + (num_frames - 2)
            logger.info(
                f"  Cycling enabled: {original_frames} -> {num_frames} frames")
        ...
        # Verify all frames have same atom count and types
        num_atoms_per_frame = [len(frame.molecule)
                               for frame in trajectories[0].children]
        for traj in trajectories[1:]:
            frame_atoms = [len(frame.molecule) for frame in traj.children]
            if frame_atoms != num_atoms_per_frame:
                raise ValueError(
                    "All trajectories must have the same number of atoms per frame")

        num_atoms = num_atoms_per_frame[0]
        logger.info(f"  Atoms: {num_atoms}")

        # Get first frame for geometry
        first_frame = trajectories[0].children[0]
        molecule = first_frame.molecule
        settings = first_frame.render_settings
        ...
        # Collect all positions across all frames to find bounding box
        all_positions = np.vstack(
            [frame.molecule.positions for traj in trajectories for frame in traj.children])
        bbox_min = all_positions.min(axis=0)
        bbox_max = all_positions.max(axis=0)
        bbox_size = bbox_max - bbox_min
        max_extent = np.max(bbox_size)
        ...
        # Prepare animation names and objects for export
        trajectory_objects = trajectories
        animation_names = [getattr(
            traj, 'name', f"Trajectory_{i+1}") for i, traj in enumerate(trajectories)]
        gltf = {
            "asset": {
                "version": "2.0",
        ...
        for anim_idx, (traj, anim_name) in enumerate(zip(trajectory_objects, animation_names)):
            ...
            for atom_idx in range(num_atoms):
                translations = np.array(
                    [positions[atom_idx]
                        for positions in atom_positions_per_frame],
                    dtype=np.float32
                )
                offset, length = add_to_buffer(translations.tobytes())
    ...
    def export_multi_trajectory_animated_glb(
        ...
        **kwargs
        ...
        if animation_names is not None and len(animation_names) != len(trajectory_objects):
            raise ValueError(
        ...
        # Use trajectory names if animation_names not provided
        if animation_names is None:
            animation_names = [getattr(traj, 'name', f"Trajectory_{i+1}")
                               for i, traj in enumerate(trajectory_objects)]

        logger.info(f"Exporting multi-animation GLB to {output_path}")
        logger.info(f"  Trajectories: {len(trajectory_objects)}")
        logger.info(f"  FPS: {fps}")
        logger.info(f"  Resolution: {resolution}")

        # Verify all trajectories have frames and same atom count
        for i, traj in enumerate(trajectory_objects):
            if not traj.children:
                raise ValueError(f"Trajectory {i} has no frames")

        # Check atom counts match across all trajectories
        first_traj = trajectory_objects[0]
        first_frame = first_traj.children[0]
        num_atoms = len(first_frame.molecule)

        for i, traj in enumerate(trajectory_objects):
            for frame_idx, frame in enumerate(traj.children):
                if len(frame.molecule) != num_atoms:
                    raise ValueError(
                        f"All trajectories must have the same number of atoms. "
                        f"Trajectory {i}, frame {frame_idx} has {len(frame.molecule)} atoms, "
        ...
        # Collect all positions across all frames to find bounding box
        all_positions = np.vstack(
            [frame.molecule.positions for traj in trajectory_objects for frame in traj.children])
        bbox_min = all_positions.min(axis=0)
        bbox_max = all_positions.max(axis=0)
        bbox_size = bbox_max - bbox_min
        max_extent = np.max(bbox_size)
    ...
    def export_stl(
        self,
        output_path: Union[str, Path],
        printing_mode: bool = True,
        printing_resolution: int = 32,
        **kwargs
    ) -> None:
        """
        Export the current scene to an STL file suitable for 3D printing.

        Notes:
        - STL does not support vertex colors or transparency; exported mesh will be
          a watertight geometry suitable for slicers.
        - This method prefers the same mesh-generation path used for GLB printing mode,
          ensuring center-to-center bonds and increased resolution.
        ...
        if output_path.suffix.lower() not in ['.stl', '.obj']:
            raise ValueError(
                f"Invalid file extension '{output_path.suffix}'. Must be '.stl' or '.obj'."
            )

        mode_str = "3D printing (STL)" if printing_mode else "visualization (STL)"
        logger.info(f"Exporting scene to {output_path} ({mode_str})")

        # Collect trimesh meshes (colors are ignored for STL)
        meshes_with_colors = self._collect_meshes_with_colors(
            printing_mode=printing_mode,
            printing_resolution=printing_resolution
        )
        ...
        # Take only trimesh objects and concatenate
        trimesh_list = [tm for tm, _ in meshes_with_colors]
        try:
            if len(trimesh_list) == 1:
                combined = trimesh_list[0]
            else:
                combined = trimesh.util.concatenate(trimesh_list)

            # If printing_mode, try to produce a single watertight solid mesh
            if printing_mode and not combined.is_watertight:
                logger.info(
                    "Printing mode: attempting to produce watertight solid for STL")

                # 1) Try boolean union (best quality) if engine available
                union_success = False
                try:
                    # trimesh.boolean.union may require an external engine (scad/blender)
                    logger.debug(
                        "Attempting boolean union of parts (may require external engine)")
                    union_mesh = trimesh.boolean.union(
                        trimesh_list, engine=None)  # engine None lets trimesh pick
                    if union_mesh is not None and union_mesh.is_watertight:
                        combined = union_mesh
                        union_success = True
                        logger.info(
                            "Boolean union succeeded and produced watertight mesh")
                except Exception as e:
                    logger.debug(f"Boolean union failed: {e}")

                # 2) Fallback: voxelization -> marching cubes (robust solid)
                if not union_success:
                    try:
                        # Choose a reasonable pitch based on bounding box and printing_resolution
                        bbox = combined.bounds if hasattr(
                            combined, 'bounds') else combined.bounding_box.bounds
                        extents = bbox[1] - bbox[0]
                        max_extent = float(
                            max(extents)) if extents.size else 1.0
                        # Higher printing_resolution -> smaller pitch -> finer voxels
                        pitch = max_extent / \
                            max(32.0, float(printing_resolution) * 4.0)
                        logger.debug(
                            f"Voxelizing mesh with pitch={pitch:.6f} (resolution={printing_resolution})")

                        vox = _voxel_creation.voxelize(combined, pitch=pitch)
                        # marching_cubes returns a trimesh.Trimesh
                        voxel_mesh = vox.marching_cubes
                        if voxel_mesh is not None and voxel_mesh.vertices.size and voxel_mesh.is_watertight:
                            combined = voxel_mesh
                            logger.info(
                                "Voxelization -> marching_cubes produced watertight mesh")
                        else:
                            logger.debug(
                                "Voxel marching_cubes produced non-watertight or empty mesh")
                    except Exception as e:
                        logger.debug(f"Voxelization fallback failed: {e}")

                # 3) Final best-effort repair if still not watertight
                if not combined.is_watertight:
                    try:
                        logger.debug(
                            "Attempting mesh repair (fill_holes / remove degenerate faces / merge vertices)")
                        repaired = combined.copy()
                        try:
                            repaired.fill_holes()
                        except Exception:
                            pass
                        try:
                            _trimesh_repair.fix_normals(repaired)
                        except Exception:
                            pass
                        try:
                        ...
                        try:
                            ...
                            pass
                        try:
                            repaired.merge_vertices()  # best-effort
                        except Exception:
                            pass

                        if repaired.is_watertight:
                            combined = repaired
                            logger.info("Mesh repair produced watertight mesh")
                        else:
                            logger.warning(
                                "Could not make mesh fully watertight; STL may contain holes")
                    except Exception as e:
                        logger.debug(f"Final repair attempt failed: {e}")

            # Export. trimesh chooses binary STL by default for .stl
            combined.export(str(output_path))

            logger.info(f"✅ Successfully exported STL to {output_path}")
            logger.info(f"   Total vertices: {combined.vertices.shape[0]}")
            logger.info(f"   Total faces: {combined.faces.shape[0]}")
        except Exception as e:
